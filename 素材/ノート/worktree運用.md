...
- 設計スプリント（第1章のキャラ・組織・敵・評価基盤を承認待ちドラフト化。characters / organizations / story の3レーン）:
  起動コマンドと手順 → `素材/ノート/第1章設計スプリント.md`（2026-08-02 作成。執筆解禁より前に回す工程）
...
- auto-writing:
  ...
  （手順の正本 → `素材/ノート/auto-writingプレイブック.md`・2026-08-02 作成）
- auto-verify:
...
MEMORY.md の規則により【未定】の設定で本文を書いてはならない。以下が確定するまで**本文の執筆には入らない**（2026-08-02 更新: このチェックはプレイブックの「執筆解禁チェック」としてループ1周ごとに行う。未達の間、auto-writing は文体サンプル・章設計叩き台などの準備作業と判断キューの積み上げに回るため、ループ自体は起動してよい）:
